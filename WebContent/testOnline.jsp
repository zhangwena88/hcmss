			...
			$(function(){
				var vm = new Vue({
					...
					methods:
					{
						...
						clickSelectTopic:function(){
							console.log(3);
							this.showaddQuestionBox = false;
							this.showselectQuestionBox = false;
							this.showselectTopicBox= true;
							 var url = "http://localhost:8080/hcms/topic/findAll.action";
							$.get(url,function(data){
								console.log(data);
								vm.topicList=data;
						...
						update2:function(index){
							var url = "http://localhost:8080/hcms/topic/update.action";
							var item = vm.topicList[index];
							console.log(item);
							var param = {"topic.topic_id":item.topic_id,
										 "topic.topic_subject":item.topic_subject,
										 "topic.topic_choice":item.topic_choice,
										 "topic.teacher_id":item.teacher_id
										};
							$.get(url,param,function(data){
								console.log(data);
							},"json");
						...
						delect2:function(index){
							var url = "http://localhost:8080/hcms/topic/delect.action";
							var item = vm.topicList[index];
							console.log(item);
							var param = {"topic.topic_id":item.topic_id,
										 "topic.topic_subject":item.topic_subject,
...
<body>
	<div id="app">
			...
			<button id="selectTopic" @click="clickSelectTopic()">查看试卷</button>
			<div id="showBox">
				...
				<div id="selectQuestionBox" v-show="showselectQuestionBox">
					...
					<input type="button" value="刷新" @click="renovate('question')"/>
				</div>
				
				
				<div id="selectTopicBox" v-show="showselectTopicBox">
					<table id="topicTable" cellspacing="0">
						<tr>
							<th>试卷id</th>
							<th>试卷唯一标识</th>
							<th>所含试题编号</th>
						...
						<tr v-for="topic,index in topicList">
							<td><textarea type="text" v-model="topic.topic_id" ></textarea></td>
							<td><textarea type="text" v-model="topic.topic_subject" ></textarea></td>
							<td><textarea type="text" v-model="topic.topic_choice" ></textarea></td>
							<td><textarea type="text" v-model="topic.teacher_id" ></textarea></td>
							<td><a @click="update2(index)">修改</a></td>
							<td><a @click="delect2(index)">删除</a></td>
							
						</tr>
					</table> 
					
					<input type="button" value="刷新" @click="renovate('topic')"/>
				</div>
			</div>
		</div>
</body>
